...
def run(cmd):
    print(f"ἰ0 Ejecutando: {' '.join(cmd)}")
...
# =======================
# Verificar contenedores (mejorado)
# =======================

def contenedor_activo(nombre):
    try:
        resultado = subprocess.run(["docker", "ps", "--format", "{{.Names}}"], capture_output=True, text=True)
        contenedores_activos = resultado.stdout.strip().split("\n")
        return any(nombre in c for c in contenedores_activos)
    except Exception as e:
...
def check_containers(dry_run: bool = False) -> None:
    ...
    if dry_run:
        ...
        return
    for svc in SERVICES:
        if contenedor_activo(svc):
            print(f"✅ Contenedor activo: {svc}")
        else:
            print(f"⚠️ No encontrado o detenido: {svc}")
...
def run_main(dry_run: bool = False) -> None:
    ...
    scripts = [SCRIPTS_DIR / "download_models.sh"]
